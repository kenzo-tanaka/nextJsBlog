...
アクセストークンの仕組みによって、ユーザは日程調整サービスにGoogleの認証情報を提供しないで済む。

また、ユーザが承認して初めて日程調整サービスはGoogleカレンダーのリソースに対して、限定的なアクセスが可能になる。OAuth 2.0では、「`データはサービスプロバイダーではなくユーザのものである`」という考え方に基づいているため、ユーザの承認なしにリソースが提供されることはない。  
