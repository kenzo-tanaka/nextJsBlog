...
なのでまとめると、通常使っているフルリゾルバーはISPが提供している顧客ネットワーク内のみにアクセス制御されたものです。

## パブリックDNSサービス

オープンリゾルバーはアクセス制限がされておらず、インターネットのどのネットワークからでも名前解決サービスを使えるようになっているフルリゾルバーのことです。

GoogleのPublic DNSなどはオープンリゾルバーの中で、管理者がセキュリティ対策などを実施したうえで意図して使えるようにしたパブリックDNSサービスに分類されます。

パブリックDNSサービスはGoogleのPublic DNS以外に以下のようなものがあります。
...
- Slackの今回の障害はDNSが原因
- 一部のユーザが使用しているフルリゾルバーからの名前解決ができなかった
- Googleが公開しているパブリックDNSサービスを使用することで名前解決ができるようになった

## 参考

[DNSがよくわかる教科書 Kindle版](https://amzn.to/2WuqYXQ)
